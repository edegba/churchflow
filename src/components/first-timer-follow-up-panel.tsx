...
export function FirstTimerFollowUpPanel({
  firstTimerId,
  firstTimerName,
}: {
  firstTimerId: string;
  ...
  const { data: tasks, isLoading } = useFirstTimerFollowUps(firstTimerId);
    ...
    <div className="space-y-4">
      ...
      {all.length === 0 ? (
        <EmptyState
          ...
          description="Create a follow-up to start tracking care for this first timer."
        />
      ) : (
        <>
          <Card>
            ...
            <CardContent className="divide-y py-0">
              {open.length === 0 ? (
                <p className="py-4 text-sm text-muted-foreground">
                  No open follow-ups — this first timer is all caught up.
                </p>
              ) : (
                open.map((t) => (
                  <FollowUpRow
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-2">
            <Label htmlFor="ft-completion-note">Completion note (optional)</Label>
            <Textarea
              id="ft-completion-note"
              rows={3}
